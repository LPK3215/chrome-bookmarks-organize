...
本技能本质是一份**提示词**（`SKILL.md`）。搭配 `scripts/bm.py` 更稳，但要用脚本必须把本仓库下载下来；如果只想让 AI 按规范发挥，**只加载技能即可**（支持联网加载），不用下载脚本。

| 用法 | 你要做什么 | 适合 | 代价 |
|---|---|---|---|
| **仅技能 / 提示词** | 把 `SKILL.md` 加载进 AI（skill 市场联网装、Claude Code 等加载 skill、或直接把文件交给 AI）。**不 clone、不下载 `scripts/`。** AI 按同一套 0→7 流程自己读写 Bookmarks JSON。 | 试手、一次性小改、不想装本地仓库 | 没有脚本闸门：备份、退浏览器、删 checksum、写 UTF-8 全靠 AI 临场做对 |
| **技能 + 脚本（推荐）** | clone / 下载本仓库，让 AI 调 `scripts/bm.py`。体检、底牌、闸门、写回、校验、还原都锁在脚本里，一次写对、每次一样。 | 真实书签、要反复整理、要可还原 | 多一步下载 |

`SKILL.md` 里写了判定：工作区有 `scripts/bm.py` → 走脚本；没有（只加载了技能）→ 自动切到仅提示词，并套用那一节的硬约束。有脚本却手改 JSON = 丢掉护栏，不要这么干。
...
| `finalize` | **唯一动真身**：先验候选与**目标**合法性（目标解析不出 `roots` 也拒，防误盖别的文件）+ 删 checksum + 改名陈旧 `.bak` + **原子写回**（先 .tmp 再 replace）；浏览器在跑/Profile 被占用拒绝（除非 `--force`），写前自动再兜一份；**`--from` 与 `--target` 同一文件则拒（防自我覆盖）** |
...
```bash
python scripts/test/run_tests.py     # 46 项，零第三方依赖，约 1 秒
```
...
- `scripts/test/run_tests.py` — 零依赖回归测试（46 项），改完 `bm.py` 请跑它。
