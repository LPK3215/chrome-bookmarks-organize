...
本技能**默认用脚本执行**：工作区有 `scripts/bm.py` 就必走脚本（闸门在代码里），**禁止绕开它手改 JSON**。只有拿不到脚本（联网加载、只贴了本文件）才降级「仅提示词」。仓库根目录另有 `PROMPT.md`——把整套流程压成一段可直接复制给任意聊天 AI 的纯文本提示词，给"不 clone 仓库、只想顺手整理一次/先体验/拿去分享"的人用；它是**没有脚本闸门**的轻路径，不是正式整理方式。

动手前先看**本技能所在目录**有没有 `scripts/bm.py`：

| 判定 | 用法 | 怎么做 |
|---|---|---|
| **有** `scripts/bm.py` | **脚本模式（默认 · 推荐）** | 走下方「步骤」里的 `python scripts/bm.py …`。安全闸门、备份、写回、校验都锁在脚本里。 |
